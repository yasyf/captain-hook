#!/bin/bash
# GENERATED by cc-guides from .claude/fragments/captain_hook/scripts/install-binary.sh/ — do not edit; edit the fragments and push — CI renders.
# Locate the binrun runner and exec the hook descriptor.
#
# This renders to plugin scripts/install-binary.sh — the successor to the
# provision-a-symlink installer. bin/hook is a committed symlink to it, so
...
# resolves and execs the version-exact artifact the sidecar bin/hook.binrun
...
if [ -e "$ROOT/.orphaned_at" ] && is_version "${ROOT##*/}"; then
  ...
  for sibling in "${ROOT%/*}"/*/; do
    ...
    [ ! -e "$sibling/.orphaned_at" ] && [ -x "$sibling/bin/hook" ] || continue
    if version_gt "${sibling##*/}" "${newest##*/}"; then
      newest=$sibling
    fi
  done
  [ "$newest" = "$ROOT" ] || exec "$newest/bin/hook" "$@"
fi

DESCRIPTOR="$ROOT/bin/hook.binrun"
