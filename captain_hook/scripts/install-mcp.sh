#!/bin/bash
# GENERATED by cc-guides from .claude/fragments/captain_hook/scripts/install-mcp.sh/ — do not edit; edit the fragments and push — CI renders.
# Locate the binrun runner and exec the capt-hook descriptor.
#
# This renders to plugin scripts/install-binary.sh — the successor to the
# provision-a-symlink installer. bin/capt-hook is a committed symlink to it, so
...
# resolves and execs the version-exact artifact the sidecar bin/capt-hook.binrun
...
if [ -e "$ROOT/.orphaned_at" ] && is_version "${ROOT##*/}"; then
  ...
  for sibling in "${ROOT%/*}"/*/; do
    ...
    [ ! -e "$sibling/.orphaned_at" ] && [ -x "$sibling/bin/capt-hook" ] || continue
    if version_gt "${sibling##*/}" "${newest##*/}"; then
      newest=$sibling
    fi
  done
  [ "$newest" = "$ROOT" ] || exec "$newest/bin/capt-hook" "$@"
fi

DESCRIPTOR="$ROOT/bin/capt-hook.binrun"
