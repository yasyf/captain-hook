#!/bin/bash
# GENERATED by cc-guides from .claude/fragments/captain_hook/scripts/install-hook-shim.sh/ — do not edit; edit the fragments and push — CI renders.
# Locate the binrun runner and exec the hook-shim descriptor.
#
# This renders to plugin scripts/install-binary.sh — the successor to the
# provision-a-symlink installer. bin/hook-shim is a committed symlink to it, so
...
# resolves and execs the version-exact artifact the sidecar bin/hook-shim.binrun
...
ROOT="$(cd "$d/.." && pwd)"
DESCRIPTOR="$ROOT/bin/hook-shim.binrun"
