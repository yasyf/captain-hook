...
class Matcher:
    ...
    module: ClassVar[Matcher]
    cls: ClassVar[Matcher]
    func: ClassVar[Matcher]
    definition: ClassVar[Matcher]
    imports: ClassVar[Matcher]
    call: ClassVar[Matcher]
    assignment: ClassVar[Matcher]
    control_flow: ClassVar[Matcher]
    type_checking: ClassVar[Matcher]
    future_annotations: ClassVar[Matcher]
    forward_ref: ClassVar[Matcher]
    any: ClassVar[Matcher]
    private: ClassVar[Matcher]
    dunder: ClassVar[Matcher]
    constant: ClassVar[Matcher]

    @classmethod
    def kind(cls, *types: type[ast.AST], label: str | None = None) -> Matcher:
        """Match any of the given AST node ``types`` — the primitive for a category not shipped."""
        return cls(lambda node, _parents: isinstance(node, types), label or "|".join(t.__name__ for t in types))

    @classmethod
    def calls(cls, name: str) -> Matcher:
    ...
    @classmethod
    def kwarg(cls, name: str) -> Matcher:
    ...
    @classmethod
    def ref(cls, name: str) -> Matcher:
        """Match the bare name reference ``name`` (e.g. ``ref("Any")`` for ``x: Any``)."""
        return cls(lambda node, _parents: isinstance(node, ast.Name) and node.id == name, f"ref({name})")

    @classmethod
    def named(cls, pattern: str | re.Pattern[str]) -> Matcher:
    ...
    @classmethod
    def annotated(cls, inner: Matcher | None = None) -> Matcher:
        ...
        return cls(test, f"annotated({inner.label})" if inner else "annotated", structural=True)

    @classmethod
    def under(cls, other: Matcher) -> Matcher:
        ...
        return cls(test, f"under({other.label})", structural=True)

    @classmethod
    def child_of(cls, other: Matcher) -> Matcher:
    ...
    @classmethod
    def following(cls, boundary: Matcher) -> Matcher:
        ...
        return cls(test, f"following({boundary.label})", structural=True)
...
Matcher.module = Matcher.kind(ast.Module, label="module")
Matcher.cls = Matcher.kind(ast.ClassDef, label="class")
Matcher.func = Matcher.kind(*FUNCTIONS, label="function")
Matcher.definition = Matcher.cls | Matcher.func
Matcher.imports = Matcher.kind(ast.Import, ast.ImportFrom, label="import")
Matcher.call = Matcher.kind(ast.Call, label="call")
Matcher.assignment = Matcher.kind(*ASSIGNMENTS, label="assignment")
Matcher.control_flow = Matcher.kind(*CONTROL_FLOW, label="control-flow")
Matcher.type_checking = Matcher(lambda node, _parents: is_type_checking(node), "type-checking")
Matcher.future_annotations = Matcher(lambda node, _parents: has_future_annotations(node), "future-annotations")
Matcher.forward_ref = Matcher(
    lambda node, parents: (
...
Matcher.any = Matcher(lambda _node, _parents: True, "any")
Matcher.private = Matcher.named(r"^_(?!_)")
Matcher.dunder = Matcher.named(r"^__\w+__$")
Matcher.constant = Matcher.named(r"^_?[A-Z][A-Z0-9_]*$")
