...
| `gate` | `(message, *, when=None, signals=None, only_if=(), skip_if=(), events=None, max_fires=-1, tests=None, async_=False, skip_planning_agents=None)` | `Stop \| SubagentStop`; blocks, defaults to **unlimited** fires (keeps enforcing); `skip_if` is taken verbatim — a gate that should hold while background work runs passes `Waiting()` itself |
