...
The matcher catalog for [styleguide rules](../guide/styleguide.qmd). A matcher is a node
predicate. It wraps a `(node, parents) -> bool` test, so `M.calls("zip")` asks "is this AST
