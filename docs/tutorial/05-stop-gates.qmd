...
`TouchedFile` and `UsedSkill` read the session's history, not the current tool call — that's what makes gating "done" possible at all. The same machinery drives test gates (`RanCommand(Regex(r"\bpytest\b"))`) and artifact checks.
