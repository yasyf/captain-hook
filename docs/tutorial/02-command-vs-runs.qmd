...
The conservative match has a cost: `git commit -m "git stash"` blocks too, and that's a command you actually want to run. capt-hook gives you both ends of the tradeoff. Page one's `block_command` sits at one end; here's the other — the same guard rebuilt on `Runs`, which matches the parsed argv of each command in the line:
