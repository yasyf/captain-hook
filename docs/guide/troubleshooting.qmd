...
Every event runs `~/.daemonkit/bin/capt-hookd`, which `capt-hook helper install` copies out of
`~/Applications/Captain Hook.app`. If that client or the app is missing, dispatch exits non-zero
...
Legacy dispatch entries. The only consumer command is `"$HOME/.daemonkit/bin/capt-hookd" hook
