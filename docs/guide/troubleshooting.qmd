...
- See [Configuration](configuration.qmd) for the full settings surface.
...
It writes one JSONL line per event under the log directory ([see Configuration](configuration.qmd#log-directory)). Tail the file while the agent runs to see the exact `tool_input`, `tool_name`, and metadata your hooks will receive.
