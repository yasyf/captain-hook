...
Last, hunt down legacy `capt-hook run <Event>` entries. Claude Code dedupes hook commands per registering source, not globally, so a `run` entry that survives anywhere besides the captain-hook plugin — a hand-wired line in a project's `.claude/settings.json`, or a pack-shipping consumer plugin built before the attach-only contract — dispatches the event again in a sibling process. A once-guard collapses the duplicates on side-effecting events — the first sibling wins and the rest exit silently — but decision events are exempt, since swallowing a sibling there could bypass a gate. The tell is doubled verdicts on `PreToolUse` or `Stop`: two permission prompts for one tool call, or the same block message landing twice. Delete hand-wired entries from the project settings yourself; a legacy consumer plugin sheds its `run` entries when it releases against the attach-only contract in [Ship a pack in a Claude plugin](plugin-packs.qmd), since the plugin cache re-pulls on a version bump.
