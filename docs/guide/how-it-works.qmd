...
The signed Go client ensures the one exact host is running, then sends the event over daemonkit's
...
The first event starts the host from the same fixed signed executable. Daemonkit owns listener
takeover, admission, draining, process identity, and ordered shutdown. An exact newer build
replaces the prior host generation; there is no watchdog polling files or versions in the
background.
...
`brew upgrade --cask` (retried as a forced reinstall, which also repairs a half-removed app) and
...
`brew install --cask yasyf/tap/captain-hook`. Dispatch fails non-zero — the version probe has
