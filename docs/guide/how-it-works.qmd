...
The framework has two exact-version parts: the `capt-hook` wheel published to PyPI and the Developer ID-signed, notarized `Captain Hook.app` published through the Homebrew cask. The plugin resolves the wheel with `uvx --isolated --from capt-hook` and its `hook` shim execs `/Applications/Captain Hook.app/Contents/Helpers/capt-hookd`. You trust the maintainers, PyPI, the GitHub release, and the Homebrew tap as one release path.
...
`uvx --isolated --from capt-hook hook` runs the wheel's tiny dispatch shim from an ephemeral environment that `uv` builds on demand. The Python worker and its dependencies come from that same environment, separate from your project. Your project's virtualenv stays untouched: captain-hook never lands in your `pyproject.toml`, your lockfile, or your site-packages, so you can add a hook to any repository without changing a single dependency in it.

Both `--isolated` and `--from capt-hook` are load-bearing. They select the `hook` executable from a fresh resolution of the named distribution instead of an unrelated installed command. A cache hit needs no network, so hooks still fire offline. The shim sends its exact wheel version and Python executable to the signed host; the host rejects any version mismatch before Python dispatch. Isolation prevents accidental tool capture, while the build fence prevents the cache and app from drifting silently.
...
The wheel's `hookd` command is only an exec shim for the same helper, so
`uvx --isolated --from capt-hook hookd status` is equivalent. Status and control commands never
start a missing host. Only event dispatch ensures it is running.
...
`brew install --cask yasyf/tap/captain-hook`. The shim fails non-zero and names the exact missing
path; it does not run the Python CLI instead.

**The wheel and app builds differ.** Upgrade the cask and captain-hook plugin to the same release,
then run `/reload-plugins` or start a new Claude Code session. Exact build rejection is expected
until both artifacts agree.

**A worker is stuck or stale.** Run `uvx --isolated --from capt-hook hookd status`, inspect
`capt-hookd.log`, then use `uvx --isolated --from capt-hook hookd restart-workers`. Restart kills
and reaps the current worker generations; it is not a cache invalidation hint or a graceful
watchdog cycle.
