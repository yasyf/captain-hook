...
length-framed persistent transport. When no host is listening, or the host is starting or
draining, the client waits for a ready host within its own deadline and sends once. The client
...
Because the host resolves the wheel from its own build, the upgrade is complete on its own: the
next worker materializes the matching wheel automatically. Set
