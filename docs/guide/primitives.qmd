...
[`StyleRule`](../../reference/style.StyleRule.qmd) subclass whose docstring is the message and whose
`match` is built from the [`matchers`](../../reference/style.matchers.qmd) module:
...
`only_if` / `skip_if` / `events` / `block`. See [Style Rules](styleguide.qmd) for the full guide,
...
::: {.callout-note title="More primitives"}
For LLM-powered hooks (`llm_gate`, `llm_nudge`, `prompt_check`), see [LLM Hooks](llm-hooks.qmd).
For multi-step enforcement, see [Workflows](workflows.qmd).
:::
