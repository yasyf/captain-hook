---
title: "Writing hooks"
description: "Primitives, conditions, and style rules — the vocabulary every hook is built from."
...
Signals score across the whole window by default (`scope="window"`); `scope="text"` requires one message to reach the threshold on its own. `T` builds transcript fixtures in the exact shape Claude Code writes — `T.user`/`T.assistant` lines carrying text, `T.tool` calls, `T.result` blocks — and raw JSONL dicts (`{"type": ..., "message": {"content": [...]}}`) mix in freely for shapes `T` doesn't cover; [Query the transcript](llm-hooks.qmd#query-the-transcript) shows how to capture a real one. A nudge with `signals` defaults to `PostToolUse`; without them it defaults to `PreToolUse`. See [Signals](llm-hooks.qmd#score-patterns-with-signals) for the scoring model.
...
```python
from captain_hook import Ask, Block, FromSubagent, Input, Runs, deny
...
The two command conditions do different jobs. `Runs` names the command structurally, so a subagent writing `echo "git push --force"` into a file never trips the deny; `Command` then matches the one piece of text no argv prefix names, the flag, with `(?!-)` keeping `--force-with-lease` out. Reaching for `Command(r"git\s+push\s+--force")` alone is what makes a guard fire on its own documentation.
...
[Ship a pack in a Claude plugin](packs.qmd#ship-a-pack-in-a-plugin) shows the plugin-tier setup this primitive was built for.
...
```python
from captain_hook import hook, Event, RanCommand, Runs, Tool
...
| A bash command by name | `Runs("git", "push")` — structural, no false fires |
| Command text no argv prefix names | `Command(r"curl.*\|\s*sh")` — a pipeline, redirect, or flag |
...
The first five inspect the current event; `TouchedFile` and `RanCommand` inspect the session transcript, so they tell you whether something already happened. The last two are typed custom-condition bases, covered below. For every condition with its pattern type and defaults, see the [conditions reference](../reference/conditions.qmd).
...
::: {.callout-warning title="Name a command with `Runs`, not `Command`"}
`Command` searches the raw command line, so `Command(r"git\s+stash")` fires on
`echo "git stash"` and on a heredoc that quotes the phrase. `Runs("git", "stash")`
compares argv tokens, so a mention never matches. Constructing a `Command` whose pattern is
a plain command-name prefix warns with the `Runs` spelling to use. Keep `Command` for what
an argv prefix cannot name — `Command(r"curl.*\|\s*sh")`, a redirect, a flag's value.
:::

::: {.callout-tip title="Regex patterns"}
`Tool`, `Command`, and `Content` match with regex — escape special characters and use
raw strings (`r"..."`) to avoid backslash issues. `Runs` and `RanCommand` take argv tokens
instead. `RanCommand` matching is wrapper-transparent (`sudo`/`env`/`timeout` are stripped)
but launcher-literal, so list `RanCommand("uv", "run", "pytest")` and `RanCommand("pytest")`
as separate entries.
:::
...
```python
from captain_hook import hook, Event, InPlanMode, Or, Runs, Tool
...
hook(
    ...
    only_if=[
        Tool("Bash"),
        Or(Runs("git", "rebase"), Runs("git", "reset"), Runs("git", "push", "--force")),
    ],
    skip_if=[InPlanMode()],
)
```
...
[Inspect and rewrite commands](commands.qmd) owns the command primitives and the fluent walk. [LLM hooks](llm-hooks.qmd) covers the model-backed primitives, [Workflows](state.qmd#enforce-a-multi-step-workflow) multi-step enforcement, and the [reference](../reference/primitives.qmd) every full signature.
:::
