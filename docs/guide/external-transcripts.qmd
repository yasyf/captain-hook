...
The install step matters: an MCP server is a long-running process, and a `uvx` parent would hold uv's global cache lock for the server's whole lifetime, stalling every other uv operation on the machine.
