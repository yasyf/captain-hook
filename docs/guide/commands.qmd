...
`block_command` reads its first argument by shape. A token list is argv and lowers to `Runs`, which catches the command anywhere in a `&&` chain or pipeline and never fires on a line that only quotes the words.
...
Pass a string when you mean text no argv prefix names — `r"git\s+push\s+--force(?!-)"` blocks a force-push while allowing `--force-with-lease`, and `r"curl.*\|\s*sh"` guards a pipeline. A string compiles to a `Command` regex over the raw line, which also reaches text the shell parser cannot structure, such as a comment or a malformed line. It reaches quoted mentions too, so scope it accordingly.

One token list keeps the regex: `["rm", "-rf", "*"]`. The `"*"` stands for a word that must be there, and `Runs` matches an argv *prefix*, so lowering it would widen the block onto the bare `rm -rf` it used to let through.
...
`RanCommand`'s arguments are argv tokens, matched as a leading prefix: wrapper-transparent (`sudo`/`env`/`timeout` are stripped) but launcher-literal, so `RanCommand("pytest")` does not match `uv run pytest`. Listing every launcher is what `Regex` avoids. One `RanCommand(Regex(r"\bpytest\b"))` covers `pytest`, `uv run pytest` and `poetry run pytest`, where the literal form needs an entry each and silently misses the one you forgot.

Opting in is the `Regex` call, never the shape of the string, so a literal token holding metacharacters (`a.out`, `c++`) keeps matching literally. The trade is that a regex matches command *text*: it searches each parsed command's argv join, so a heredoc body cannot match it, but `echo pytest` can. Where that decides whether a gate stands down, list the literal spellings instead.
