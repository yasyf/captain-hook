...
`RanCommand` takes argv tokens, never a regex. Matching is wrapper-transparent (`sudo`/`env`/`timeout` are stripped) but launcher-literal — `RanCommand("uv", "run", "pytest")` and `RanCommand("pytest")` are separate entries, which is why both appear above. A regex handed to it is read as a literal token, so `RanCommand(r"\bpytest\b")` looks for a command named `\bpytest\b` and can never match; on a gate's `skip_if` that is an exemption which silently never fires.
