...
llm_nudge(
    ...
    signals=Signals(
        patterns=[
            Signal(pattern=r"should contain", weight=2),
            Signal(pattern=r"probably|I think", weight=1),
            Signal(pattern=r"let me (read|check|look)", weight=-2),
        ],
        threshold=3,
        window=10,
    ),
...
def security_review(evt):
    verdict = llm_evaluate(
        ...
        SecurityVerdict,
        signals=Signals(
            patterns=[Signal(pattern=r"\.env|credentials|secret", weight=3)],
            threshold=2,
        ),
    )
