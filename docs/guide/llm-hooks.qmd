...
Signals(
    ...
    threshold=3,  # fire when the window's union score reaches 3
...
By default (`scope="window"`) union scoring counts each distinct matching signal's weight once across the whole window, however many entries it matches — so a tell that splits across adjacent messages still accumulates: "retry" in one message and "again" in another sum to 3 and fire, just as "Let me retry that again" does in a single message.

Pooling across turns can misfire when quoted or referenced vocabulary sums to the threshold. When co-occurrence in a single message is the tell, opt into per-text scoring with `scope="text"`: each text is scored on its own, the hook fires only when one text reaches the threshold alone, and the window only bounds how far back that text may sit. Under either scope, a `vetoes` match anywhere in the window holds the hook back.
