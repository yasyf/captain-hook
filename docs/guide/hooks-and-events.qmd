...
| `only_if` / `skip_if` | Condition lists — see [Conditions](conditions.qmd) |
...
# lint takes a check function (string- or AST-mode), not a name + regex
def no_print(content: str) -> list[str]:
    return [ln for ln in content.splitlines() if "print(" in ln]

lint(no_print, message="Use logger instead: {violations}")
```

See the [Primitives Guide](primitives.qmd) for the full list.
...
::: {.callout-tip title="Which pattern should I use?"}
...
Conditions filter *when* a hook fires. See the dedicated [Conditions guide](conditions.qmd) for the full reference, or the [Conditions Cheatsheet](../reference/conditions.qmd) for a quick-reference card.
...
See [How It Works](../getting-started/how-it-works.qmd) for a detailed step-by-step walkthrough with actual JSON input/output.
