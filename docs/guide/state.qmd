...
When a hook spans several events, you don't want each handler hand-rolling the same read-update-write. Decorate the model with `@workflow_state("name")` and it grows three methods that take the event directly. `.load(evt)` reads and defaults to a fresh instance, `.save(evt)` writes, and `.reset(evt)` deletes.
