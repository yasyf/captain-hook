...
committed `.claude/settings.json`. The shim execs `~/.daemonkit/bin/capt-hookd hook <Event>`, so
those hooks run on the installed app's wheel, not this checkout's `.venv`. To exercise checkout
code live, build and install the app from it.
