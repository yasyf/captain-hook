...
Each page picks one capability of the framework, frames it as a problem you've actually hit, and links to a self-contained `examples/*.py` file you can drop into your hooks directory. Every example file ships with inline `tests = {...}` you can run via `captain-hook test --hooks packages/captain-hook/examples`.
