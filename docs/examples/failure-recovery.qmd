...
This page composes two `nudge` primitives: a `signals`-driven Stop nudge that scores the agent's narration, and a `PostToolUseFailure` nudge gated on the turn's failure count. The retry bundle's tells span attempts — one retry phrase per failed try pools toward the threshold under the default `scope="window"`, where `scope="text"` would need a single message to reach it alone.
...
- [Signals guide](../guide/llm-hooks.qmd#score-patterns-with-signals)
