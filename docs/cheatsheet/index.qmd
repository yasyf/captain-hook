...
**Runs** — a parsed command's argv starts with these tokens; how you name a command
```python
Runs("git", "stash")
```

**Command** / **Content** — the raw Bash line, or the written content, matches a regex; reach for `Command` only for text no argv prefix names
```python
Command(r"curl.*\|\s*sh")
Content(r"\bprint\(")
```
...
**Signals** — a bundle that fires when distinct signals pooled across the last
`window` events score `threshold`; `scope="text"` requires a single message to score it alone,
