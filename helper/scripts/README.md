...
| `build-local.sh [version]` | Builds the universal Go host, generates the project, builds Release, signs with your Developer ID, verifies the embedded host and bridge identities, exact versions, architectures, and hardened runtime, then applies the same daemonkit deployment transaction as the formula. |
...
`build-local.sh` installs to `~/Applications` and converges LaunchAgents. Run it only
