...
def create_candidate(store: ReviewStore, *, rule: str = "no-force-push") -> int:
...
def pack_fix_candidate(store: ReviewStore, *, origin: RepoKey = ORIGIN_REPO, pack: str = "general") -> int:
    return store.ensure_candidate(
        REPO,
        kind=CandidateKind.FIX,
        rule="misfire",
...
async def seed_merge_pair(store: ReviewStore, *, slug: str = "shared-slug") -> tuple[int, int]:
    first = create_candidate(store, rule=digest_rule("a"))
    second = create_candidate(store, rule=digest_rule("b"))
    seed(store, first, "ka", session="s1", occurred="2026-06-01T10:00:00+00:00")
    seed(store, second, "kb", session="s2", occurred="2026-06-02T10:00:00+00:00")
...
async def eligible_create_candidate(store: ReviewStore) -> int:
    candidate_id = create_candidate(store)
    for i, (session, day) in enumerate([("s1", "2026-06-01"), ("s2", "2026-06-01"), ("s3", "2026-06-02")]):
        seed(store, candidate_id, f"k{i}", session=session, occurred=f"{day}T10:00:00+00:00")
        await judge(store, f"k{i}")
    return candidate_id


def open_pr(store: ReviewStore, *, rule: str, opened_at: datetime, n: int, repo: RepoKey = REPO) -> int:
    candidate_id = create_candidate(store, rule=rule)
    store.transition(
...
async def accepted_fix(store: ReviewStore, *, resolved_at: str) -> int:
    candidate_id = fix_candidate(store)
    seed(
        store,
        candidate_id,
        "pre",
...
class TestCandidates:
    def test_ensure_create_candidate_groups_by_repo_and_rule(self, store: ReviewStore) -> None:
        first = create_candidate(store, rule="no-force-push")
        assert create_candidate(store, rule="no-force-push") == first
        assert create_candidate(store, rule="prefer-uv-run") != first
        other_repo = store.ensure_candidate(
        ...
        assert (candidate_row(store, first))["status"] == "watching"

    def test_ensure_fix_candidate_groups_by_target(self, store: ReviewStore) -> None:
        first = fix_candidate(store)
        again = store.ensure_candidate(
            REPO,
            kind=CandidateKind.FIX,
            rule="different-rule",
            source_kind=SourceKind("hook_complaint"),
        ...
        assert again == first
        assert fix_candidate(store, file=".claude/hooks/other.py") != first
    ...
    def test_ensure_candidate_rejects_invalid_target_shape(
        self, store: ReviewStore, kwargs: dict[str, object]
    ) -> None:
        with pytest.raises(sqlite3.IntegrityError):
            store.ensure_candidate(REPO, **kwargs)
...
class TestTransitions:
    ...
    def test_transition_matrix(self, store: ReviewStore, frm: CandidateStatus, to: CandidateStatus) -> None:
        candidate_id = create_candidate(store)
        for step in PATHS[frm]:
            store.transition(candidate_id, step, pr_opened_at=datetime.now(UTC))
        if (frm, to) in ALLOWED_MOVES:
            store.transition(candidate_id, to, pr_opened_at=datetime.now(UTC))
            assert (candidate_row(store, candidate_id))["status"] == str(to)
        else:
            with pytest.raises(InvalidTransition, match=f"{frm} -> {to}"):
                store.transition(candidate_id, to)
            assert (candidate_row(store, candidate_id))["status"] == str(frm)

    def test_pr_open_stamps_url_and_kept_through_acceptance(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        ...
        row = candidate_row(store, candidate_id)
    ...
    def test_transition_unknown_candidate_raises(self, store: ReviewStore) -> None:
        with pytest.raises(LookupError, match="no candidate with id 999"):
            store.transition(999, CandidateStatus.PR_OPEN)

    def test_accepted_stamps_supplied_merge_time_not_wall_clock(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        store.transition(candidate_id, CandidateStatus.PR_OPEN, pr_url="https://github.com/x/y/pull/1")
        merged_at = datetime(2026, 7, 8, 15, 6, 25, tzinfo=UTC)
        store.transition(candidate_id, CandidateStatus.ACCEPTED, resolved_at=merged_at)
        assert (candidate_row(store, candidate_id))["resolved_at"] == "2026-07-08T15:06:25+00:00"

    def test_stale_write_cannot_overwrite_a_concurrent_acceptance(self, tmp_path: Path) -> None:
        # The peer's acceptance lands just before the staler's CAS UPDATE: the staler loses,
        # re-reads ACCEPTED, and rejects.
        path = tmp_path / "review.db"
        with ReviewStore.open(path) as setup:
            candidate_id = create_candidate(setup)
            setup.transition(candidate_id, CandidateStatus.PR_OPEN, pr_opened_at=datetime.now(UTC))
        ...
        try:
            ...
            def gated(sql: str, *args: object, **kwargs: object) -> object:
                nonlocal accepted
                if sql.startswith("UPDATE candidates SET status") and not accepted:
                    accepted = True
    ...
    def test_identical_concurrent_transition_converges_instead_of_raising(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            def gated(sql: str, *args: object, **kwargs: object) -> object:
                nonlocal raced
                if sql.startswith("UPDATE candidates SET status") and not raced:
                    raced = True
    ...
    def test_direct_same_state_transition_stays_invalid(self, store: ReviewStore) -> None:
        ...
        candidate_id = create_candidate(store)
        store.transition(candidate_id, CandidateStatus.PR_OPEN, pr_opened_at=datetime.now(UTC))
        store.transition(candidate_id, CandidateStatus.ACCEPTED)
        with pytest.raises(InvalidTransition, match="accepted -> accepted"):
            store.transition(candidate_id, CandidateStatus.ACCEPTED)

    def test_stale_pr_result_cannot_resolve_a_reopened_candidate(self, store: ReviewStore) -> None:
        ...
        assert (candidate_row(store, candidate_id))["status"] == "accepted"


class TestCreateEligibility:
    def test_unjudged_observations_never_count(self, store: ReviewStore, settings: ReviewSettings) -> None:
        store.enable(REPO)
        candidate_id = create_candidate(store)
        for i, day in enumerate(("2026-06-01", "2026-06-02", "2026-06-03")):
            seed(store, candidate_id, f"k{i}", session=f"s{i}", occurred=f"{day}T10:00:00+00:00")
        status = store.threshold_status(candidate_id, settings=settings)
        assert (status.sessions, status.days) == (0, 0)
        assert store.eligible(candidate_id, settings=settings) is False
    ...
    async def test_only_confident_judge_acceptance_counts(
        ...
        assert store.eligible(candidate_id, settings=settings) is counts
    ...
    async def test_three_observations_in_one_session_are_one_session(
        ...
        candidate_id = create_candidate(store)
        for i, day in enumerate(("2026-06-01", "2026-06-02", "2026-06-03")):
            seed(store, candidate_id, f"k{i}", session="s1", occurred=f"{day}T10:00:00+00:00")
            await judge(store, f"k{i}")
        status = store.threshold_status(candidate_id, settings=settings)
    ...
    async def test_three_sessions_on_one_utc_day_are_one_day(
        ...
        candidate_id = create_candidate(store)
        for i in range(3):
            seed(store, candidate_id, f"k{i}", session=f"s{i}", occurred=f"2026-06-01T1{i}:00:00+00:00")
            await judge(store, f"k{i}")
        status = store.threshold_status(candidate_id, settings=settings)
    ...
    async def test_days_count_in_utc(self, store: ReviewStore, settings: ReviewSettings) -> None:
        store.enable(REPO)
        candidate_id = create_candidate(store)
        seed(store, candidate_id, "k0", session="s0", occurred="2026-06-01T10:00:00+00:00")
        seed(store, candidate_id, "k1", session="s1", occurred="2026-06-02T01:00:00+03:00")
    ...
    async def test_eligible_when_sessions_and_days_cross_thresholds(
        ...
        assert store.eligible(candidate_id, settings=settings) is True
    ...
    async def test_non_watching_status_never_eligible(
        ...
        if terminal != CandidateStatus.PR_OPEN:
            store.transition(candidate_id, terminal)
        status = store.threshold_status(candidate_id, settings=settings)
    ...
    async def test_other_prompt_version_verdicts_never_count(
        ...
        store.enable(REPO)
        candidate_id = await eligible_create_candidate(store)
        with ReviewStore.open(
            tmp_path / "review.db",
            versions=PromptVersions(create=store.versions.create + 1, fix=store.versions.fix + 1),
        ) as bumped:
            status = bumped.threshold_status(candidate_id, settings=settings)
            assert (status.sessions, status.days) == (0, 0)
            assert bumped.eligible(candidate_id, settings=settings) is False
...
class TestPrCap:
    async def test_cap_blocks_when_open_prs_reach_max(self, store: ReviewStore, settings: ReviewSettings) -> None:
        store.enable(REPO)
        candidate_id = await eligible_create_candidate(store)
        open_pr(store, rule="other-a", opened_at=datetime.now(UTC), n=1)
        open_pr(store, rule="other-b", opened_at=datetime.now(UTC), n=2)
        status = store.threshold_status(candidate_id, settings=settings)
    ...
    async def test_stale_transition_frees_a_slot(self, store: ReviewStore, settings: ReviewSettings) -> None:
        store.enable(REPO)
        candidate_id = await eligible_create_candidate(store)
        stale_one = open_pr(store, rule="other-a", opened_at=datetime.now(UTC), n=1)
        open_pr(store, rule="other-b", opened_at=datetime.now(UTC), n=2)
        store.transition(stale_one, CandidateStatus.STALE)
        status = store.threshold_status(candidate_id, settings=settings)
    ...
    async def test_pr_older_than_stale_after_days_frees_a_slot(
        ...
        store.enable(REPO)
        candidate_id = await eligible_create_candidate(store)
        open_pr(store, rule="other-a", opened_at=datetime.now(UTC) - timedelta(days=31), n=1)
        open_pr(store, rule="other-b", opened_at=datetime.now(UTC), n=2)
        status = store.threshold_status(candidate_id, settings=settings)
        assert status.open_prs == 1
        assert store.eligible(candidate_id, settings=settings) is True
    ...
    async def test_other_repos_prs_never_count(self, store: ReviewStore, settings: ReviewSettings) -> None:
        ...
        for rule in ("other-a", "other-b"):
            other = store.ensure_candidate(
                RepoKey("github.com/other/repo"),
                kind=CandidateKind.CREATE,
                rule=rule,
                source_kind=SourceKind("transcript_message"),
            )
            store.transition(other, CandidateStatus.PR_OPEN, pr_opened_at=datetime.now(UTC))
        status = store.threshold_status(candidate_id, settings=settings)
    ...
    async def test_cross_target_pr_counts_against_target_not_origin(
        ...
        origin_candidate = store.ensure_candidate(
            ORIGIN_REPO,
            kind=CandidateKind.CREATE,
            rule="cross-target",
            source_kind=SourceKind("transcript_message"),
        ...
        assert store.open_pr_targets(settings=settings) == {REPO: 1}
        assert (store.threshold_status(target_candidate, settings=settings)).open_prs == 1
        assert (store.threshold_status(origin_candidate, settings=settings)).open_prs == 0

    def test_shared_pr_url_counts_once_against_target(self, store: ReviewStore, settings: ReviewSettings) -> None:
        open_pr(store, rule="shared-one", opened_at=datetime.now(UTC), n=7)
...
class TestFixEligibility:
    ...
    async def test_single_observation_path_requires_very_high_and_judge_acceptance(
        ...
        assert store.eligible(candidate_id, settings=settings) is expected
...
class TestThresholdStatus:
    ...
    def test_unknown_candidate_raises(self, store: ReviewStore, settings: ReviewSettings) -> None:
        with pytest.raises(LookupError, match="no candidate with id 999"):
            store.threshold_status(999, settings=settings)
...
class TestRegroup:
    async def test_merge_reparents_shared_slug_and_sweeps_husks(self, store: ReviewStore) -> None:
        ...
        assert store.regroup_create() == (2, 0)
        [survivor] = store.candidates()
    ...
    async def test_second_regroup_is_idempotent_and_byte_identical(self, store: ReviewStore) -> None:
        ...
        candidates, observations = (
            dump_table(store, "candidates"),
            dump_table(store, "candidate_observations"),
        )
        assert store.regroup_create() == (0, 0)
        assert dump_table(store, "candidates") == candidates
        assert dump_table(store, "candidate_observations") == observations
    ...
    async def test_terminal_candidates_are_immune(self, store: ReviewStore, terminal: CandidateStatus) -> None:
        candidate_id = create_candidate(store, rule=digest_rule("term"))
        seed(store, candidate_id, "kt", session="s1", occurred="2026-06-01T10:00:00+00:00")
        await judge(store, "kt", slug="different-slug")
        for step in PATHS[terminal]:
            store.transition(candidate_id, step, pr_opened_at=datetime.now(UTC))
        assert store.regroup_create() == (0, 0)
        [row] = store.candidates()
    ...
    async def test_fidelity_upgrade_moves_observation_never_duplicates(self, store: ReviewStore) -> None:
        digest = create_candidate(store, rule=digest_rule("f"))
        seed(store, digest, "kf", session="s1", occurred="2026-06-01T10:00:00+00:00")
        await judge(store, "kf", slug="slug-a", fidelity="summary")
        assert store.regroup_create() == (1, 0)
        [on_a] = store.candidates()
        assert (on_a["rule"], on_a["observations"]) == ("slug-a", 1)
        await judge(store, "kf", slug="slug-b", fidelity="full")
        assert store.regroup_create() == (1, 0)
        [on_b] = store.candidates()
        assert (on_b["rule"], on_b["observations"]) == ("slug-b", 1)
        assert len(dump_table(store, "candidate_observations")) == 1

    async def test_fix_lane_and_unjudged_create_are_immune(self, store: ReviewStore) -> None:
        fix_id = fix_candidate(store)
        seed(store, fix_id, "kf", session="s1", occurred="2026-06-01T10:00:00+00:00")
        await judge(store, "kf", slug="would-be-slug")
        create_id = create_candidate(store, rule=digest_rule("unj"))
        seed(store, create_id, "kc", session="s2", occurred="2026-06-01T10:00:00+00:00")
        assert store.regroup_create() == (0, 0)
        rows = {int(str(r["id"])): r for r in store.candidates()}
    ...
    async def test_all_rejected_retires_while_mixed_stays_watching(self, store: ReviewStore) -> None:
        ...
        rejected_id = create_candidate(store, rule=digest_rule("rej"))
        for i, (session, day) in enumerate([("s1", "2026-06-01"), ("s2", "2026-06-02")]):
            seed(store, rejected_id, f"r{i}", session=session, occurred=f"{day}T10:00:00+00:00")
            await judge(store, f"r{i}", accepted=False)
        mixed_id = create_candidate(store, rule=digest_rule("mix"))
        seed(store, mixed_id, "m0", session="s3", occurred="2026-06-01T10:00:00+00:00")
        seed(store, mixed_id, "m1", session="s4", occurred="2026-06-02T10:00:00+00:00")
        await judge(store, "m0", accepted=False)
        assert store.regroup_create() == (0, 1)
        retired = store.candidate(rejected_id)
        assert (retired["status"], retired["observations"], retired["sample_text"]) == ("rejected", 2, "text r0")
        assert (candidate_row(store, mixed_id))["status"] == "watching"
    ...
    async def test_new_observation_attaches_to_terminal_and_stays_ineligible(
        ...
        store.enable(REPO)
        slug = "durable-rule"
        candidate_id = create_candidate(store, rule=slug)
        for i, (session, day) in enumerate([("s1", "2026-06-01"), ("s2", "2026-06-01"), ("s3", "2026-06-02")]):
            seed(store, candidate_id, f"e{i}", session=session, occurred=f"{day}T10:00:00+00:00")
            await judge(store, f"e{i}", slug=slug)
        assert store.eligible(candidate_id, settings=settings) is True
        store.transition(candidate_id, CandidateStatus.PR_OPEN, pr_opened_at=datetime.now(UTC))
        store.transition(candidate_id, CandidateStatus.ACCEPTED)
        assert create_candidate(store, rule=slug) == candidate_id
        seed(store, candidate_id, "fresh", session="s9", occurred="2026-06-05T10:00:00+00:00")
...
class TestPerLaneVersions:
    async def test_create_and_fix_lanes_resolve_verdicts_independently(
        ...
        with ReviewStore.open(tmp_path / "review.db", versions=bumped_create) as lanes:
            fix_status = lanes.threshold_status(fix_id, settings=settings)
            assert fix_status.single_observation is True
            assert lanes.eligible(fix_id, settings=settings) is True

            stale = lanes.threshold_status(create_id, settings=settings)
            assert (stale.sessions, stale.days) == (0, 0)
            assert lanes.eligible(create_id, settings=settings) is False

            for i in range(3):
                await judge(lanes, f"k{i}")
            live = lanes.threshold_status(create_id, settings=settings)
            assert (live.sessions, live.days) == (3, 2)
            assert lanes.eligible(create_id, settings=settings) is True

    def test_judge_queue_concatenates_create_then_fix(self, store: ReviewStore) -> None:
        for key in ("ka", "kb", "kc"):
            candidate_id = create_candidate(store, rule=digest_rule(key))
            seed(store, candidate_id, key, session="s0", occurred="2026-06-01T10:00:00+00:00")
        complaint_id = fix_candidate(store)
        seed(
            store,
            complaint_id,
            "kh",
        ...
        assert [str(row["dedup_key"]) for row in store.judge_queue()] == ["ka", "kb", "kc", "kh"]

    def test_judge_backlog_skips_the_hydration_probe(
        self, store: ReviewStore, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        probes: list[bool] = []

        def fake_unjudged(*, probe_hydration: bool = True, **_: object) -> list[dict[str, object]]:
    ...
    def test_judge_queue_probes_hydration_by_default(
        self, store: ReviewStore, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        probes: list[bool] = []
        ...
        monkeypatch.setattr(store, "unjudged", fake_unjudged)
        store.judge_queue(refresh_summary=True)
    ...
    async def test_judge_health_recency_is_lane_exact(self, store: ReviewStore, tmp_path: Path) -> None:
        ...
        with ReviewStore.open(tmp_path / "review.db", versions=bumped_both) as bumped:
            assert (bumped.judge_health()).last_verdict_at is None
            complaint_id = fix_candidate(bumped)
            seed(
                bumped,
                complaint_id,
                "kh",
            ...
            assert (bumped.judge_health()).last_verdict_at is not None
...
class TestPurgeStaleVerdicts:
    async def test_purge_deletes_stale_version_rows_and_spares_live(self, store: ReviewStore) -> None:
        ...
        store.purge_stale_verdicts()
        after = {(str(r["dedup_key"]), int(r["prompt_version"])) for r in dump_table(store, "verdicts")}
    ...
    async def test_purge_spares_other_roles(self, store: ReviewStore) -> None:
        ...
        store.purge_stale_verdicts()
        after = {(str(r["role"]), int(r["prompt_version"])) for r in dump_table(store, "verdicts")}
        assert after == {("auditor", 1)}

    async def test_purge_is_lane_aware_between_create_and_fix(self, store: ReviewStore) -> None:
        correction_id = create_candidate(store, rule="corr-rule")
        seed(store, correction_id, "kc", session="s0", occurred="2026-06-01T10:00:00+00:00")
        complaint_id = fix_candidate(store)
        seed(
            store,
            complaint_id,
...
class TestPurgeGating:
    ...
    async def test_same_fingerprint_reopen_skips_purge(self, store: ReviewStore, tmp_path: Path) -> None:
        ...
        with ReviewStore.open(tmp_path / "review.db") as reopened:
            after = {int(r["prompt_version"]) for r in dump_table(reopened, "verdicts")}
    ...
    async def test_purge_runs_once_per_fingerprint_change(self, store: ReviewStore, tmp_path: Path) -> None:
        ...
        with ReviewStore.open(tmp_path / "review.db", versions=bumped) as again:
            assert {int(r["prompt_version"]) for r in dump_table(again, "verdicts")} == {bumped.create - 1}


class TestMigration:
    WAVE_COLUMNS = {"generation", "resolved_at", "origin_repo_key", "pack_name", "announced_status"}

    def test_open_adds_wave_columns_to_a_fresh_db(self, store: ReviewStore) -> None:
        assert self.WAVE_COLUMNS <= candidate_columns(store)

    def test_prewave_db_gains_columns_and_backfills_both_defaults(self, tmp_path: Path) -> None:
        ...
        with ReviewStore.open(path) as store:
            assert self.WAVE_COLUMNS <= candidate_columns(store)
            rows = {str(row["rule"]): row for row in dump_table(store, "candidates")}
    ...
    def test_reopen_skips_migration_and_never_rebackfills(self, tmp_path: Path) -> None:
        path = tmp_path / "prewave.db"
...
class TestResolvedAtStamping:
    def test_accepting_stamps_resolved_at_matching_updated_at(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        store.transition(candidate_id, CandidateStatus.PR_OPEN, pr_opened_at=datetime.now(UTC))
        store.transition(candidate_id, CandidateStatus.ACCEPTED)
        row = candidate_row(store, candidate_id)
    ...
    def test_non_accepting_transitions_leave_resolved_at_null(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        store.transition(candidate_id, CandidateStatus.PR_OPEN, pr_opened_at=datetime.now(UTC))
        assert (candidate_row(store, candidate_id))["resolved_at"] is None
        store.transition(candidate_id, CandidateStatus.STALE)
        assert (candidate_row(store, candidate_id))["resolved_at"] is None
    ...
    async def test_reopen_transition_keeps_resolved_at(self, store: ReviewStore) -> None:
        ...
        store.transition(candidate_id, CandidateStatus.WATCHING)
        assert (candidate_row(store, candidate_id))["resolved_at"] == "2026-06-15T00:00:00+00:00"
...
class TestReopenRecurrentFixes:
    async def test_recurrence_after_resolution_reopens_bumps_generation_and_keeps_pr_url(
        ...
        assert store.reopen_recurrent_fixes() == 1
        row = candidate_row(store, candidate_id)
    ...
    async def test_merge_time_resolution_counts_between_merge_and_sync_recurrence(self, store: ReviewStore) -> None:
        ...
        store.transition(candidate_id, CandidateStatus.ACCEPTED, resolved_at=datetime(2026, 7, 8, tzinfo=UTC))
        seed(
            store,
            candidate_id,
            "recur",
    ...
    async def test_create_kind_recurrence_is_ignored(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store, rule="durable-rule")
        seed(store, candidate_id, "cpost", session="s-c", occurred="2026-07-01T10:00:00+00:00")
...
class TestReopenedThresholdGating:
    ...
    async def test_plain_accepted_candidate_counts_all_evidence(
        ...
        store.transition(candidate_id, CandidateStatus.ACCEPTED)
        status = store.threshold_status(candidate_id, settings=settings)
        assert (int((candidate_row(store, candidate_id))["generation"]), status.sessions, status.days) == (
            1,
            3,
            2,
        )
...
class TestJudgePassReopenWiring:
    async def test_judge_pass_surfaces_reopened_count(
        ...
        assert (candidate_row(store, candidate_id))["status"] == "watching"


class TestCrossRepoVisibility:
    def test_pack_fix_visible_from_both_origin_and_target_repo(self, store: ReviewStore) -> None:
        candidate_id = pack_fix_candidate(store)
        assert [int(str(row["id"])) for row in store.candidates(ORIGIN_REPO)] == [candidate_id]
        assert [int(str(row["id"])) for row in store.candidates(REPO)] == [candidate_id]
        assert store.candidates(RepoKey("github.com/other/thing")) == []

    def test_row_carries_routing_provenance(self, store: ReviewStore) -> None:
        pack_fix_candidate(store)
        [row] = store.candidates(ORIGIN_REPO)
        assert (row["repo_key"], row["origin_repo_key"], row["pack_name"]) == (REPO, ORIGIN_REPO, "general")

    def test_repo_local_fix_has_no_origin_and_hides_cross_repo(self, store: ReviewStore) -> None:
        local_id = fix_candidate(store)
        assert (candidate_row(store, local_id))["origin_repo_key"] is None
        assert [int(str(row["id"])) for row in store.candidates(REPO)] == [local_id]
        assert store.candidates(ORIGIN_REPO) == []
...
class TestOriginWatchingGate:
    ...
    async def test_eligibility_follows_the_origin_repo_not_the_pack_repo(
        ...
        store.enable(REPO)
        gated = store.threshold_status(candidate_id, settings=settings)
        assert (gated.watching, gated.single_observation) == (False, True)
        assert store.eligible(candidate_id, settings=settings) is False

        store.enable(ORIGIN_REPO)
        watched = store.threshold_status(candidate_id, settings=settings)
...
class TestFeedbackMigration:
    def test_open_adds_triage_column_to_a_fresh_db(self, store: ReviewStore) -> None:
        assert "triage" in feedback_columns(store)

    def test_prewave_feedback_events_gains_triage_column(self, tmp_path: Path) -> None:
        ...
        with ReviewStore.open(path) as store:
            assert "triage" in feedback_columns(store)
            assert triage_of(store, "k") is None


class TestJunkTriage:
    def test_record_triage_marks_the_junk_keys(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        seed(store, candidate_id, "junk", session="s1", occurred="2026-06-01T10:00:00+00:00")
        seed(store, candidate_id, "keep", session="s2", occurred="2026-06-02T10:00:00+00:00")
        store.record_triage(DedupKey("junk"), junk=True)
        store.record_triage(DedupKey("keep"), junk=False)
        assert store.junk_triaged_keys() == {"junk"}
        assert (triage_of(store, "junk"), triage_of(store, "keep")) == (TRIAGE_JUNK, TRIAGE_KEEP)

    def test_reject_junk_triaged_retires_all_junk_and_spares_mixed(self, store: ReviewStore) -> None:
        all_junk = create_candidate(store, rule=digest_rule("all"))
        seed(store, all_junk, "a1", session="s1", occurred="2026-06-01T10:00:00+00:00")
        seed(store, all_junk, "a2", session="s2", occurred="2026-06-02T10:00:00+00:00")
        mixed = create_candidate(store, rule=digest_rule("mix"))
        seed(store, mixed, "m1", session="s1", occurred="2026-06-01T10:00:00+00:00")
        seed(store, mixed, "m2", session="s2", occurred="2026-06-02T10:00:00+00:00")
        untriaged = create_candidate(store, rule=digest_rule("untriaged"))
        seed(store, untriaged, "u1", session="s1", occurred="2026-06-01T10:00:00+00:00")

        for key in ("a1", "a2", "m1"):
            store.record_triage(DedupKey(key), junk=True)
        store.record_triage(DedupKey("m2"), junk=False)

        assert store.reject_junk_triaged() == 1
        assert (candidate_row(store, all_junk))["status"] == CandidateStatus.REJECTED
        assert (candidate_row(store, mixed))["status"] == CandidateStatus.WATCHING
        assert (candidate_row(store, untriaged))["status"] == CandidateStatus.WATCHING

    def test_untriaged_create_events_scopes_to_watching_create(self, store: ReviewStore) -> None:
        watching = create_candidate(store, rule=digest_rule("w"))
        seed(store, watching, "w1", session="s1", occurred="2026-06-01T10:00:00+00:00")
        triaged = create_candidate(store, rule=digest_rule("t"))
        seed(store, triaged, "t1", session="s2", occurred="2026-06-01T10:00:00+00:00")
        store.record_triage(DedupKey("t1"), junk=False)
        rejected = create_candidate(store, rule=digest_rule("r"))
        seed(store, rejected, "r1", session="s3", occurred="2026-06-01T10:00:00+00:00")
        store.transition(rejected, CandidateStatus.REJECTED)
        fix = fix_candidate(store)
        seed(store, fix, "f1", session="s4", occurred="2026-06-01T10:00:00+00:00", source_kind="hook_complaint")

        rows = store.untriaged_create_events(limit=10)
        assert [row["dedup_key"] for row in rows] == ["w1"]

    def test_untriaged_create_events_respects_the_limit(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        for i in range(5):
            seed(store, candidate_id, f"k{i}", session=f"s{i}", occurred="2026-06-01T10:00:00+00:00")
        assert len(store.untriaged_create_events(limit=3)) == 3

    def test_judge_queue_excludes_junk_triaged_events(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        seed(store, candidate_id, "junk", session="s1", occurred="2026-06-01T10:00:00+00:00")
        seed(store, candidate_id, "keep", session="s2", occurred="2026-06-02T10:00:00+00:00")
        store.record_triage(DedupKey("junk"), junk=True)
        keys = {str(row["dedup_key"]) for row in store.judge_queue()}
        assert keys == {"keep"}
    ...
    def test_record_triage_is_a_compare_and_set(
        self, store: ReviewStore, first_junk: bool, second_junk: bool, landed: str
    ) -> None:
        candidate_id = create_candidate(store)
        seed(store, candidate_id, "e0", session="s1", occurred="2026-06-01T10:00:00+00:00")
        assert store.record_triage(DedupKey("e0"), junk=first_junk) is True
        assert store.record_triage(DedupKey("e0"), junk=second_junk) is False
        assert triage_of(store, "e0") == landed

    def test_kept_evidence_is_never_rejected_by_a_losing_junk_write(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store)
        seed(store, candidate_id, "e0", session="s1", occurred="2026-06-01T10:00:00+00:00")
        assert store.record_triage(DedupKey("e0"), junk=False) is True
        assert store.record_triage(DedupKey("e0"), junk=True) is False
        assert store.reject_junk_triaged() == 0
        assert (candidate_row(store, candidate_id))["status"] == CandidateStatus.WATCHING

    async def test_untriaged_create_events_excludes_judge_ruled_events(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store, rule="canonical-slug")
        seed(store, candidate_id, "e0", session="s1", occurred="2026-06-01T10:00:00+00:00")
        assert [row["dedup_key"] for row in store.untriaged_create_events(limit=10)] == ["e0"]
        await judge(store, "e0")
        assert store.untriaged_create_events(limit=10) == []

    async def test_reject_junk_triaged_spares_accepted_judge_evidence(self, store: ReviewStore) -> None:
        candidate_id = create_candidate(store, rule="canonical-slug")
        seed(store, candidate_id, "e0", session="s1", occurred="2026-06-01T10:00:00+00:00")
        await judge(store, "e0")
        assert store.record_triage(DedupKey("e0"), junk=True) is True
        assert store.reject_junk_triaged() == 0
        assert (candidate_row(store, candidate_id))["status"] == CandidateStatus.WATCHING
    ...
    async def test_judge_acceptance_revives_a_junk_rejected_candidate(self, store: ReviewStore) -> None:
        ...
        assert store.record_triage(DedupKey("e0"), junk=True) is True
        assert store.reject_junk_triaged() == 1
        assert (candidate_row(store, candidate_id))["status"] == CandidateStatus.REJECTED

        await judge(store, "e0", accepted=True)
        assert store.revive_junk_rejected() == 1
        assert (candidate_row(store, candidate_id))["status"] == CandidateStatus.WATCHING

    def test_revive_ignores_junk_rejected_without_a_judge_acceptance(self, store: ReviewStore) -> None:
        ...
        candidate_id = create_candidate(store, rule="canonical-slug")
        seed(store, candidate_id, "e0", session="s1", occurred="2026-06-01T10:00:00+00:00")
        store.record_triage(DedupKey("e0"), junk=True)
        assert store.reject_junk_triaged() == 1
        assert store.revive_junk_rejected() == 0
        assert (candidate_row(store, candidate_id))["status"] == CandidateStatus.REJECTED
    ...
    async def test_revive_spares_a_judge_rejected_candidate(self, store: ReviewStore) -> None:
        ...
        candidate_id = create_candidate(store, rule="canonical-slug")
        seed(store, candidate_id, "e0", session="s1", occurred="2026-06-01T10:00:00+00:00")
        store.record_triage(DedupKey("e0"), junk=False)
        await judge(store, "e0", accepted=True)
